...
- Deterministic per-lineage colour derived from the existing FNV hash — substrate dots, lineage tree rows, population-trajectory bands, any future panel all read the same lineage with the same hue. Strengthens the cross-panel mental model with no new state at the seam. ~10 lines of code in a shared `lineageColor(id)` helper. #r2 #ui #pulled-from-prototype
...
- Persistent action bar in the lineage inspector — Apply patch / Queue decree / Quarantine pinned above a tab strip (Drift / Firmware / Interventions) rather than living in a row that scrolls with the content. Refactor of `LineageInspectorPanel`; makes the intervention surface considerably more discoverable on first contact. #r2 #ui #pulled-from-prototype
