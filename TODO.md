...
- Configure-on-newRun in the Worker host: mint a fresh OPFS runId per newRun (or reuse for Resume) so the dashboard's Save / Load buttons can persist runs without piling up on a fixed runId #r0 #host
