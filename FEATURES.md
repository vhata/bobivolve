...
- ✓ Forensic replay — clicking a row in the events timeline rewinds the run to that event's tick; the host loads the latest in-run snapshot at-or-before the target, replays any logged commands, and pauses on the rewound state. Destructive: post-rewind state is forfeit (Save first if it's worth keeping). The timeline surfaces two strata: milestones (extinction, patch saturation, patch application, decree firing, auto-pause) always appear; speciations are filtered to the ones whose parent already holds ≥5% of population or is quarantined, with retroactive promotion when a small candidate clade grows to ≥1%, plus a "show all speciations" toggle as the player's escape hatch
- ✓ Pinned ancestry groups — follow up to six named ancestry roots across future descendants, with stable colours and population counted under the nearest pinned ancestor. Searchable, paginated membership keeps individual genetic lineages inspectable; pins are browser/run preferences, not intervention targets or named-save contents.
